...
`apply_plan` accepts one validated rectangular room on a matching blank canvas and creates it as one native rectangular wall through Dungeondraft's wall tool. It does not paint terrain, place objects, modify arbitrary existing geometry, or expose a network service. A successful result remains editable in Dungeondraft and can be reversed with the application's normal manual Undo command.
